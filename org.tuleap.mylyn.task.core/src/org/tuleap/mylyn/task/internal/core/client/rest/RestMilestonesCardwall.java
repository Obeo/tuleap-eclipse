...
/**
 * JSON Resource for the {@code /api/<version>/milestones/:milestoneId/cardwall} URL.
 * 
 * @author <a href="mailto:laurent.delaigue@obeo.fr">Laurent Delaigue</a>
 */
public class RestMilestonesCardwall extends AbstractAuthenticatedRestResource {
	 ...
	 */
	protected RestMilestonesCardwall(String serverUrl, String apiVersion, ICredentials credentials,
			int milestoneId) {
	...
	protected String getUrl() {
		return URL.MILESTONES + "/" + milestoneId + URL.CARDWALL; //$NON-NLS-1$
	}

	/**
	 * Sends an GET request to the {@code /api/<version>/milestones/:milestoneId/cardwall} URL and returns the
	 * response.
	 * 
	 * @param headers
	...
	/**
	 * Sends an OPTIONS request to the {@code /api/<version>/milestones/:milestoneId/cardwall} URL and checks
	 * that the GET operation is allowed in the response provided by the server.
	 * 
	 * @param headers
	 *            Headers to use for sending the request, just in case. There is no reason why this map
	 *            shouldn't be empty.
